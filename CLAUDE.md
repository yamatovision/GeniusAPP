...
AppGeniusのプロンプトファイルおよび技術コンテンツの保護に関する検討事項です。

### セキュリティ課題
- プロンプトファイルのような知的財産がユーザーに直接配布される形式では技術流出のリスクがある
- ClaudeCodeがファイルを読めなければならないため完全な暗号化は困難
- プロジェクトデータ（env.md, structure.mdなど）はVSCodeからも編集可能にする必要がある

### 実装可能なセキュリティソリューション

1. **API連携プロンプト配信**
   - プロンプトをローカルに保存せず、APIから必要時に取得
   - ライセンス認証ベースのアクセストークン管理
   - 初回ログイン時に長期リフレッシュトークンを発行
   - 利用時にはアクセストークンを自動更新
   - オフライン利用のための期間限定キャッシュ

2. **ハイブリッドアプローチ**
   - プロンプト部分（requirementsadvicer.md, mockup_analysis_template.mdなど）: APIから配信
   - プロジェクトデータ（env.md, structure.mdなど）: ローカル保存・編集可能
   - クライアント認証情報は安全に暗号化して保存
...
この方向性でセキュリティを実装することで、知的財産を保護しつつVSCodeとClaudeCodeの連携性を維持できます。具体的な実装は次期バージョンで検討します。
## 進捗状況
- 完成予定ファイル数: 48
- 作成済みファイル数: 18
- 進捗率: 37.5%
