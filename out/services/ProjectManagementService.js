...
`, 'utf8');
            // data_models.md
            fs.writeFileSync(path.join(projectPath, 'docs', 'data_models.md'), `# データモデル定義
...
`, 'utf8');
            // env.md
            fs.writeFileSync(path.join(projectPath, 'docs', 'env.md'), `# 環境変数リスト
...
`, 'utf8');
            // deploy.md
            fs.writeFileSync(path.join(projectPath, 'docs', 'deploy.md'), `# デプロイ情報
...
`, 'utf8');
            ...
            fs.writeFileSync(path.join(projectPath, 'docs', 'prompts', 'requirements_advisor.md'), `# 要件定義アドバイザー
...
`, 'utf8');
            ...
            fs.writeFileSync(path.join(projectPath, 'docs', 'prompts', 'scope_manager.md'), `# スコープマネージャー システムプロンプト
...
- 環境変数や外部APIの連携`, 'utf8');
            ...
            fs.writeFileSync(path.join(projectPath, 'docs', 'prompts', 'scope_implementer.md'), `# スコープ実装アシスタント
...
- スケーラビリティ考慮事項`, 'utf8');
            // デバッグ探偵プロンプト
            fs.writeFileSync(path.join(projectPath, 'docs', 'prompts', 'debug_detective.md'), `# デバッグ探偵 シャーロックホームズ - システムプロンプト
...
ワトソンくん、さあ一緒に事件を解決しましょう。まずはエラーの詳細を教えてください！`, 'utf8');
            // 環境変数アシスタント要件
            this.ensureDirectoryExists(path.join(projectPath, 'docs', 'scopes'));
            fs.writeFileSync(path.join(projectPath, 'docs', 'prompts', 'environment_manager.md'), `# 環境変数アシスタント要件定義
...
8. 一連の操作が完了するまで自動的に進行`, 'utf8');
            // CURRENT_STATUS.md - 更新済みテンプレートを使用
            fs.writeFileSync(path.join(projectPath, 'docs', 'CURRENT_STATUS.md'), `# プロジェクト名 - 実装状況 (${new Date().toISOString().split('T')[0].replace(/-/g, '/')}更新)
...
`, 'utf8');
            ...
            this.ensureDirectoryExists(path.join(projectPath, 'docs', 'scopes'));
            logger_1.Logger.info(`Created scopes directory at: ${path.join(projectPath, 'docs', 'scopes')}`);
            // docs/prompts/mockup_analyzer.md - モックアップ解析
            fs.writeFileSync(path.join(projectPath, 'docs', 'prompts', 'mockup_analyzer.md'), `# モックアップ解析と要件定義の詳細化
...
- スコープマネージャーが後工程で使用する4つの重要ドキュメント（ディレクトリ構造、データモデル、API設計、環境変数）に必要な情報を確実に含めてください`, 'utf8');
            logger_1.Logger.info(`Initial documents created for project at: ${projectPath}`);
        }
        catch (error) {
    ...
    async updateProject(id, updates) {
        try {
            ...
            try {
                const { AppGeniusEventBus, AppGeniusEventType } = await Promise.resolve().then(() => __importStar(require('./AppGeniusEventBus')));
                const eventBus = AppGeniusEventBus.getInstance();
                eventBus.emit(AppGeniusEventType.PROJECT_UPDATED, {
                    id,
                    project,
                    updates,
                    previousState
                }, 'ProjectManagementService', id);
            }
            catch (e) {
                // イベントバスが利用できなくても処理は続行
                logger_1.Logger.debug('AppGeniusEventBus not available, skipping event emission');
     ...
     * @returns 更新されたプロジェクト
     */
    async updateProjectPhase(id, phase, completed) {
        try {
            // 既存のプロジェクトを取得
            const project = this.projects.get(id);
            if (!project) {
            ...
            // メタデータファイルの更新
            await this.saveMetadata();
            logger_1.Logger.info(`Project phase updated: ${id}, ${phase}=${completed}`);
            // 状態が変わった場合だけイベント発火
            if (previousState !== completed) {
                try {
                    const { AppGeniusEventBus, AppGeniusEventType } = await Promise.resolve().then(() => __importStar(require('./AppGeniusEventBus')));
                    const eventBus = AppGeniusEventBus.getInstance();
                    eventBus.emit(AppGeniusEventType.PHASE_COMPLETED, {
                        projectId: id,
                        phase,
            ...
            logger_1.Logger.error(`Failed to update project phase: ${error.message}`);
    ...
    async deleteProject(id) {
        try {
            ...
            try {
                const { AppGeniusEventBus, AppGeniusEventType } = await Promise.resolve().then(() => __importStar(require('./AppGeniusEventBus')));
                const eventBus = AppGeniusEventBus.getInstance();
                eventBus.emit(AppGeniusEventType.PROJECT_DELETED, { id, project: deletedProject }, 'ProjectManagementService');
            }
            catch (e) {
                // イベントバスが利用できなくても処理は続行
                logger_1.Logger.debug('AppGeniusEventBus not available, skipping event emission');
     ...
     * @returns 更新されたプロジェクト
     */
    async updateProjectMetadata(id, key, value) {
        try {
            // 既存のプロジェクトを取得
            const project = this.projects.get(id);
            if (!project) {
            ...
            // メタデータファイルの更新
            await this.saveMetadata();
            logger_1.Logger.info(`Project metadata updated: ${id}, ${key}`);
            return project;
        }
        catch (error) {
            logger_1.Logger.error(`Failed to update project metadata: ${error.message}`);
            throw new Error(`プロジェクトメタデータの更新に失敗しました: ${error.message}`);
        }
    }
    /**
     * 現在アクティブなプロジェクトを取得
    ...
    getActiveProject() {
        ...
        const activeProjectId = configManager_1.ConfigManager.get('activeProjectId');
    ...
    async setActiveProject(id) {
        try {
            ...
            const project = this.projects.get(id);
            // configに保存
            await configManager_1.ConfigManager.update('activeProjectId', id, true);
            logger_1.Logger.info(`Active project set: ${id}`);
            // イベントバスが利用可能ならイベントを発火
            try {
                const { AppGeniusEventBus, AppGeniusEventType } = await Promise.resolve().then(() => __importStar(require('./AppGeniusEventBus')));
                const eventBus = AppGeniusEventBus.getInstance();
                eventBus.emit(AppGeniusEventType.PROJECT_SELECTED, project, 'ProjectManagementService');
            }
            catch (e) {
                // イベントバスが利用できなくても処理は続行
                logger_1.Logger.debug('AppGeniusEventBus not available, skipping event emission');
            ...
            logger_1.Logger.error(`Failed to set active project: ${error.message}`);
            return false;
        }
    }
    /**
    ...
    async loadProjects() {
        try {
            ...
            if (fs.existsSync(this.metadataFile)) {
                ...
                const projectList = JSON.parse(metadata);
                // メタデータをマップに設定
                projectList.forEach(project => {
                    this.projects.set(project.id, project);
                });
                logger_1.Logger.info(`Loaded ${projectList.length} projects from metadata`);
            }
            else {
                logger_1.Logger.info('No project metadata found, creating new file');
                // 新しいメタデータファイルを作成
                await this.saveMetadata();
            }
        }
    ...
    async saveMetadata() {
        try {
            ...
            logger_1.Logger.debug(`Saved metadata for ${projectList.length} projects`);
        }
        catch (error) {
