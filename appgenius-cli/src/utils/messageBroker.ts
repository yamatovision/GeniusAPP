...
export class MessageBroker {
  ...
  public sendMessage(type: MessageType, payload: any): string {
    try {
      ...
      logger.debug(`Message sent: ${messageId} (${type})`, { payload });
  ...
  private ensureWatcher(): void {
    ...
    try {
      // メッセージディレクトリを監視
      this.watcher = fs.watch(this.messageDirPath, (eventType, filename) => {
   ...
   * 進捗レポートを送信
   */
  public sendProgressReport(progress: number, currentTask: string): string {
    return this.sendMessage(MessageType.PROGRESS_REPORT, {
      progress,
      currentTask,
      timestamp: Date.now()
    });
   ...
   * スコープ更新メッセージを送信
   */
  public sendScopeUpdate(action: string, data: any = {}): string {
    return this.sendMessage(MessageType.SCOPE_UPDATE, {
      action,
   ...
   * 結果送信メッセージを送信
   */
  public sendResult(result: any): string {
    return this.sendMessage(MessageType.RESULT_SUBMIT, {
      result,
      timestamp: Date.now()
    });
  }
