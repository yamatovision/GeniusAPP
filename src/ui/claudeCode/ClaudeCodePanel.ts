...
export class ClaudeCodePanel {
  public static readonly viewType = 'appgenius.claudeCode';
  private static instance: ClaudeCodePanel | undefined;
   ...
   */
  public static createOrShow(extensionUri: vscode.Uri): ClaudeCodePanel {
  ...
  private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri) {
    ...
    this._disposables.push(
      this._authService.onAuthStateChanged(isAuthenticated => {
  ...
  private async _sendInitialData() {
    try {
      ...
      Logger.error('初期データ送信中にエラーが発生しました', error as Error);
    }
  }
  ...
  private async _sendPrompts() {
    try {
      ...
      Logger.error('プロンプト一覧取得中にエラーが発生しました', error as Error);
   ...
   * ClaudeCodeインストール
   */
  private async _installClaudeCode() {
    try {
   ...
   * プロンプトライブラリを開く
   */
  private async _openPromptLibrary() {
    try {
      // 認証確認
      if (!this._authService.isAuthenticated()) {
        const answer = await vscode.window.showInformationMessage(
   ...
   * 環境変数を更新
   */
  private async _updateEnvironmentVariables() {
    try {
      ...
      Logger.error('環境変数の更新中にエラーが発生しました', error as Error);
      vscode.window.showErrorMessage(`環境変数の更新中にエラーが発生しました: ${(error as Error).message}`);
    }
  }
   ...
   * ClaudeCode状態情報の取得
   */
  private async _getClaudeStatus() {
    try {
      const isAvailable = await this._integrationService.isClaudeCodeAvailable();
   ...
   * 同期状態情報の取得
   */
  private _getSyncStatus(justSynced: boolean = false) {
    // 認証状態を確認
    const isAuthenticated = this._authService.isAuthenticated();
   ...
   * 個別ステータス更新送信
   */
  private _sendStatusUpdate(key: string, value: any) {
    this._panel.webview.postMessage({
      type: 'statusUpdate',
      data: {
        [key]: value
  ...
  private _getHtmlForWebview(webview: vscode.Webview): string {
    ...
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'webviews', 'claudeCode', 'script.js')
    );
    const styleUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'webviews', 'claudeCode', 'style.css')
    );
