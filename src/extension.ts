	...
	try {
				...
				handleUri(uri: vscode.Uri): vscode.ProviderResult<void> {
					if (uri.path === '/launch-claude-code') {
						try {
							...
							vscode.window.showErrorMessage(`Failed to handle appgenius:// URI: ${(error as Error).message}`);
						}
					} else if (uri.path === '/auth-callback') {
						try {
							// 認証コールバックの処理
							// TODO: ここに OAuth コールバック処理を実装
							Logger.info('認証コールバックを受信しました');
							
							// 認証完了メッセージを表示
							vscode.window.showInformationMessage('認証が完了しました');
							
							// 認証成功後にダッシュボードを表示
							vscode.commands.executeCommand('appgenius-ai.openDashboard');
						} catch (error) {
							vscode.window.showErrorMessage(`Failed to handle auth callback: ${(error as Error).message}`);
						}
					}
				}
			})
			...
			vscode.commands.registerCommand('appgenius-ai.analyzeProject', async () => {
				try {
					const selectedFolder = await vscode.window.showOpenDialog({
						...
						canSelectMany: false,
						openLabel: '分析するプロジェクトフォルダを選択'
					});
					
					if (selectedFolder && selectedFolder.length > 0) {
						const folderPath = selectedFolder[0].fsPath;
						terminalInterface.showTerminal();
						terminalInterface.processQuery(`プロジェクト分析中: ${folderPath}`);
						
						const result = await projectAnalyzer.analyzeProject(folderPath);
			...
			vscode.commands.registerCommand('appgenius-ai.generateCode', async () => {
				try {
					const query = await vscode.window.showInputBox({
						prompt: '生成するコードの詳細を入力してください',
						placeHolder: '例: ユーザー認証機能を実装するReactコンポーネント'
					});
		...
		context.subscriptions.push(
			vscode.commands.registerCommand('appgenius-ai.openSimpleChat', (projectPath?: string) => {
				try {
					// 権限チェックを行う
					if (AuthGuard.checkAccess(Feature.SIMPLE_CHAT)) {
