...
export class AIService {
  ...
  constructor() {
    ...
    this.loadSavedApiKey().then(key => {
      if (key) {
        this.apiKey = key;
        Logger.debug('保存されたAPI キーを読み込みました');
      }
    }).catch(error => {
      Logger.error(`API キーのロード中にエラーが発生: ${(error as Error).message}`);
    });
  }
   ...
   * API キーを設定する
   */
  public async setApiKey(presetKey?: string): Promise<boolean> {
    ...
    if (key) {
      ...
      try {
        ...
        Logger.info('API キーが設定に保存されました');
      } catch (error) {
        Logger.error(`API キーの保存に失敗: ${(error as Error).message}`);
        return false;
      }
   ...
   * 保存されたAPIキーを読み込む
   */
  private async loadSavedApiKey(): Promise<string | undefined> {
    try {
      // 設定から読み込む
      const configKey = vscode.workspace.getConfiguration('appgeniusAI').get<string>('apiKey');
      if (configKey) {
        Logger.debug('設定からAPI キーを読み込みました');
        return configKey;
      }
      ...
      if (envApiKey) {
        Logger.debug('環境変数からAPI キーを読み込みました');
        // 環境変数から取得したキーを設定にも保存
        await this.setApiKey(envApiKey);
        return envApiKey;
      }
    } catch (error) {
      Logger.error(`API キーの読み込みに失敗: ${(error as Error).message}`);
    }
    ...
    try {
      ...
      for (const varName of envVarNames) {
        ...
        if (envKey && envKey.startsWith('sk-')) {
          Logger.debug(`環境変数 ${varName} からAPIキーを読み込みました`);
          return envKey;
        }
      }
    } catch (error) {
      Logger.error(`環境変数からのAPI キー読み込みに失敗: ${(error as Error).message}`);
    }
   ...
   */
  public async processMessage(query: string): Promise<string> {
    // 簡易コマンド処理
    if (query.startsWith('/')) {
...
`;
  ...
  public async sendMessage(
    ...
    if (!this.apiKey) {
      Logger.warn('API キーが設定されていません。APIキー設定プロセスを開始します');
      
      const success = await this.setApiKey();
      if (!success) {
        Logger.error('API キーの設定に失敗しました');
    ...
    try {
      ...
      const debugMode = vscode.workspace.getConfiguration('appgeniusAI').get<boolean>('debugMode', false); // デバッグモードはデフォルトで無効
      
      Logger.debug(`送信モード: ${mode}, デバッグモード: ${debugMode}`);
      Logger.debug(`メッセージ: ${message.substring(0, 100)}...`);
      
      if (debugMode) {
        // 開発用モックレスポンス（デバッグ目的のみ）
        Logger.info('デバッグモード: モックレスポンスを生成します');
        const response = await this.getMockResponse(message, mode);
        Logger.debug(`モックレスポンス生成完了: ${response.substring(0, 100)}...`);
        return response;
      } else {
        // 実際のAPI呼び出し
        Logger.info('実際のClaude APIを使用します');
        Logger.debug(`API キー: ${this.apiKey ? this.apiKey.substring(0, 10) + '...' : 'なし'}`);
        
        try {
          const response = await this.callClaudeApi(message, mode, systemMessages);
          Logger.debug(`API応答取得完了: ${response.substring(0, 100)}...`);
          return response;
        } catch (apiError) {
          Logger.error('Claude API呼び出しエラー:', apiError as Error);
      ...
      Logger.error('AI API処理中のエラー:', error as Error);
      Logger.debug(`エラーの詳細: ${(error as Error).stack || 'スタックトレースなし'}`);
      return `AIサービスエラー: ${(error as Error).message}。しばらくしてからもう一度お試しください。`;
    }
  }
  ...
  public async sendMessageWithStreaming(
    ...
    try {
      // デバッグ目的でのモックレスポンスの有効化
      const debugMode = vscode.workspace.getConfiguration('appgeniusAI').get<boolean>('debugMode', false);
      const useStreaming = vscode.workspace.getConfiguration('appgeniusAI').get<boolean>('useStreaming', true);
      
      Logger.debug(`送信モード: ${mode}, デバッグモード: ${debugMode}, ストリーミング: ${useStreaming}`);
      Logger.debug(`メッセージ: ${message.substring(0, 100)}...`);
      ...
      Logger.error('AI API処理中のエラー:', error as Error);
      Logger.debug(`エラーの詳細: ${(error as Error).stack || 'スタックトレースなし'}`);
      const errorMessage = `AIサービスエラー: ${(error as Error).message}。しばらくしてからもう一度お試しください。`;
   ...
   */
  private getMockResponse(message: string, mode: string): Promise<string> {
    // デバッグログにメッセージを記録
    Logger.debug(`Mock response requested for mode: ${mode}, message: ${message}`);
...
これらを踏まえて、より具体的な設計に進みましょう。どのような点に焦点を当てるべきでしょうか？`;
  ...
  private async callClaudeApi(
    ...
    try {
      ...
      if (debugMode) {
        Logger.debug('デバッグモードが有効なため、APIリクエストを送信せずモックレスポンスを返します');
      ...
      // 拡張出力設定を取得
      const useExtendedOutput = vscode.workspace.getConfiguration('appgeniusAI').get<boolean>('useExtendedOutput', false);
      ...
      if (useExtendedOutput) {
        ...
        Logger.debug(`Using extended output mode with ${adjustedMaxTokens} max tokens`);
      }
      ...
      // 思考モードが有効な場合に追加
      const useThinking = vscode.workspace.getConfiguration('appgeniusAI').get<boolean>('useThinking', false);
      if (useThinking) {
        const thinkingBudget = vscode.workspace.getConfiguration('appgeniusAI').get<number>('thinkingBudget', 8000);
        // @ts-ignore - APIの型定義がアップデートされていない可能性があるため
        requestBody.thinking = {
          type: "enabled",
          budget_tokens: thinkingBudget
        };
        Logger.debug(`Using thinking mode with budget of ${thinkingBudget} tokens`);
      }

      Logger.debug(`Sending request to Claude API: ${JSON.stringify({
        model,
        max_tokens: adjustedMaxTokens,
      ...
      try {
        ...
        const tokenCountHeaders: Record<string, string> = {
        ...
        const tokenCountResponse = await axios.post(
        ...
        Logger.debug(`Token count: ${JSON.stringify(tokenCountResponse.data)}`);
      } catch (tokenError) {
        // トークンカウントに失敗してもメインリクエストは続行
        Logger.warn(`Failed to count tokens: ${tokenError}`);
      }
      ...
      if (response.data && response.data.content && response.data.content.length > 0) {
        ...
        Logger.debug(`Received response from Claude API: ${resultText.substring(0, 100)}...`);
        return resultText;
      } else {
        Logger.error(`Unexpected API response format: ${JSON.stringify(response.data)}`);
   ...
   */
  private async callClaudeApiWithStreaming(
    message: string, 
    mode: string,
    onChunk: (chunk: string) => void,
    onComplete: (fullResponse: string) => void,
    ...
    try {
      ...
      const config = vscode.workspace.getConfiguration('appgeniusAI');
      const model = config.get<string>('model', 'claude-3-7-sonnet-20250219');
      const maxTokens = config.get<number>('maxTokens', 32000);
      
      Logger.debug(`Using AI model: ${model} with streaming mode`);
      ...
      if (useThinking) {
        ...
        requestBody.thinking = {
          type: "enabled",
          budget_tokens: vscode.workspace.getConfiguration('appgeniusAI').get<number>('thinkingBudget', 8000)
        };
      }
      ...
      Logger.debug('Making streaming API request to Claude API');
        ...
        while (buffer.includes('\n\n')) {
          ...
          for (const line of dataLines) {
            if (line.startsWith('data: ')) {
              ...
              // ストリーム終了イベント
              if (data === '[DONE]') {
                Logger.debug('Streaming complete, received [DONE]');
                onComplete(fullResponse);
                return;
              }
      ...
      Logger.error('Streaming API error:', error as Error);
      onChunk(`エラー: ${(error as Error).message}`);
      onComplete(`エラー: ${(error as Error).message}`);
    }
  }
